...
<template>
    <el-row>
        <el-col :span="24"
                :class="changeTableSize[tableSize]">
            <div class="table-outer">
                <p v-if="columns.length==0"
                   class="txt-center">{{tableColumnEmpty}}</p>
                <el-table v-if="columns.length>0"
                          ref="ListTable"
                          v-loading="loading"
                    ...
                    <template v-for="(column, index) in columns">
                        ...
                        <slot :name="column.prop"
                              v-if="!!column.template"></slot>
                    </template>
                </el-table>
            </div>
        </el-col>
        <el-col class="table-bottom-page"
                :span="24"
                v-if="pagination && pagination.totalSize>0 && columns.length>0"
                :style="{textAlign: setPagination.position}">
            <tablePage :pagination="setPagination"
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            // erd.listenTo($(".el-main div").children(), function (element) { //监听外层是否出现滚动条，如果出现滚动条重新调用设置表格宽度方法
            //     _this.reduceCall(_this.setTableWidth(), 1000)
            // })
            erd.listenTo($(".el-main>div").children(), _this.reduceCall(function () {
                _this.setTableWidth()
            }, 1000))
        ...
        methods: {
            /*tableIndex: function(index){
                return (this.setPagination.currentPage - 1) * this.setPagination.pageSize + index + 1;
            },*/
            loadData: function (searchParams) { // 获取列表数据
                ...
                if (!!_this.getDataWay.type && _this.getDataWay.type == "get") {
                    ...
                    this.$axios.get(_this.getDataWay.data, {params: Object.assign(dataParams, pageParam)})
                        .then(function (data) {
                            window.console.log(data, 'getData')
                            if (data && data.result) {
                                window.console.log(data, 'getData');
                    ...
                    this.$axios({
                        ...
                        //  window.console.log(data, 'postData')
                        if (data && data.result) {
                            // window.console.log(data, 'postData');
            ...
            setTableWidth: function () {
                ...
                if (this.$el.getElementsByClassName('list_table')[0]) {
                    ...
                    this.$el.getElementsByClassName('list_table')[0].style.width = this.tableScale + '%'
                    this.$el.getElementsByClassName('list_table')[0].style.maxWidth = this.tableScale + '%'
                }
            },
            // 防止连续调用
